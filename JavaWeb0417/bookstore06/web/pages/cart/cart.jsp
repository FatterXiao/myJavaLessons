...
<head>
    ...
    <script>
        $(function () {
            ...
            // 减少1本图书
            $(".countSubtract").click(function () {
                var input = $(this).next();
                // console.log("111111111")
                // console.log(input[0])
                var number = new Number(input.val()) - 1;
                if(number < 0){
                    number = 0
                }
                input.val(number);
                //发送请求
                location = "cartServlet?action=updateCartItem&bookId="+input.attr("id")+"&bookCount="+input.val();
            });
...
<div class="header">
    <div class="w">
        ...
        <div class="header-right">
            ...
            <div class="order"><a href="orderServlet?action=getOrders&">我的订单</a></div>&nbsp;&nbsp;
    ...
    <div class="w">
        <table>
            ...
            <c:if test="${sessionScope.cart.goodsCount > 0}">
                <c:forEach items="${sessionScope.cart.cartItems}" var="item">
                    <tr>
                        ...
                        <td>
                            <span class="countSubtract" id="subtractBook" bookId="${item.book.id} bookStock="${item.book.stock}">-</span>
                            <input class="count-num" type="text" value="${item.count}" id="${item.book.id}"/>
                            <span class="countAdd" id="addBook">+</span>
                        </td>
            ...
            <c:if test="${empty sessionScope.cart.cartItems}">
                <tr >
                    <td colspan="5" align="center" style="font-size: 30px"><a href="index.jsp">&nbsp;&nbsp;&nbsp;亲,您的购物车空空如也。快去选购吧。</a></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
        ...
        <div class="footer">
            ...
            <div class="footer-right">
                ...
                <a class="pay" href="orderServlet?action=checkOut">去结账</a>
            </div>
        </div>
    </div>
</div>
